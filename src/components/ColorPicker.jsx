...
const ColorPicker = () => {
  ...
  const [swatchColorTwo, setSwatchColorTwo] = useState('#000000');
  const [swatchColorThree, setSwatchColorThree] = useState('#000000');
  ...
  function handleClickThree(e) {
    ...
    let final_color = '#' + random_hex_color_code.slice(0, 6);
    setToggle(toggle ? null : true);
    setSwatchColorThree(final_color);
    // setIsCopied(false);
  }
      ...
      <div className='card__outer-container'>
        <div className='card__inner-container'>
          <button
            ...
            style={{ backgroundColor: swatchColorTwo }}></button>
        </div>
        <input readOnly ref={colorElement} className='card__color-code' value={swatchColorTwo} />
        {/* <button onClick={copyToClipboard}>{!isCopied ? 'Copy To Clipboard' : 'Copied'}</button> */}
      </div>
      <div className='card__outer-container'>
        <div className='card__inner-container'>
          <button
            ...
            style={{ backgroundColor: swatchColorThree }}></button>
        </div>
        <input readOnly ref={colorElement} className='card__color-code' value={swatchColorThree} />
        {/* <button onClick={copyToClipboard}>{!isCopied ? 'Copy To Clipboard' : 'Copied'}</button> */}
      </div>
    </div>
  );
